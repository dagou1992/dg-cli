...
(function (factory) {
	...
	cmd.command('install').description('初始化组件模板').action(function () {
	  return install();
	});

	var reactRender = function reactRender(fileName, suffix) {
	  return "import React, { useEffect, useState } from 'react';\n\nimport './index.".concat(suffix, "';\n\nconst ").concat(fileName, " = () => {\n  const [value, setValue] = useState(null);\n\n  useEffect(() => {}, []);\n\n  return <div>").concat(fileName, "</div>;\n};\n\nexport default ").concat(fileName, ";");
	};

	var vueRender = function vueRender(fileName, suffix) {
	  return "<template></template>\n\n<script>\nimport { reactive, toRefs } from 'vue';\nexport default {\n  name: ".concat(fileName, ",\n  setup() {\n    const obj = reactive({});\n    return {\n      ...toRefs(obj),\n    };\n  },\n};\n</script>\n\n<style ").concat(suffix === 'css' ? ' ' : "lang=\"".concat(suffix, "\" "), "scoped></style>");
	};
	...
	function _install() {
	  _install = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee() {
	    var dir,
	        frameworkList,
	        cssList,
	      ...
	      while (1) {
	        switch (_context.prev = _context.next) {
	          ...
	          case 5:
	            ...
	            cssType = _yield$inquirer$promp.cssType;
	            currentDir = path.join(dir, name);
	            spinner = ora('Creating...');
	            spinner.start();
	          ...
	          case 28:
	            ...
	            if (type === frameworkList[0]) {
	              ...
	              fs.writeFileSync(path.join(currentDir, 'index.js'), reactRender(fileName, cssType));
	              fs.writeFileSync(path.join(currentDir, "index.".concat(cssType)), '');
	              spinner.succeed();
	            }

	            if (type === frameworkList[1]) {
	              fs.writeFileSync(path.join(currentDir, 'index.vue'), vueRender(name, cssType));
	              spinner.succeed();
	            }
